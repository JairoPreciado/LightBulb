...
const LoginScreen = () => {
  ...
  // 🔹 Verificar si hay credenciales almacenadas y hacer login automático
  useEffect(() => {
    const checkStoredCredentials = async () => {
      try {
        ...
        if (storedEmail && storedPassword) {
          console.log("Autologin con:", storedEmail)
          await handleLogin(storedEmail, storedPassword, false) // Inicia sesión automáticamente
        } else {
          setLoading(false) // Si no hay credenciales, muestra la pantalla de login
        }
      } catch (error) {
  ...
  const handleLogin = async (inputEmail?: string, inputPassword?: string, saveCredentials = true) => {
    try {
      setLoading(true)

      const userCredential = await signInWithEmailAndPassword(auth, inputEmail || email, inputPassword || password)
      ...
      if (userDoc.exists()) {
        ...
        router.push("../controlLightBulb/home")

        // 🔹 Guardar credenciales solo si el usuario no viene del autologin
        if (saveCredentials) {
          await AsyncStorage.setItem("userEmail", inputEmail || email)
          await AsyncStorage.setItem("userPassword", inputPassword || password)
        }
      } else {
